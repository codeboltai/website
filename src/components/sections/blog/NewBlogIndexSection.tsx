...
export default function NewBlogIndexSection() {
  ...
  return (
    <div className="bg-background min-h-screen text-foreground font-sans selection:bg-primary selection:text-primary-foreground border-t border-border">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-8 py-24 pt-32">
        <div className="flex flex-col lg:flex-row justify-between items-end mb-24 pb-8 border-b border-border">
          <div>
            <div className="flex items-center gap-3 mb-6">
              <span className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground">Transmission Log</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-medium tracking-tight text-foreground mb-6">Research Index</h1>
        ...
        {latest && (
          <div className="mb-24">
            <div className="text-[10px] font-mono text-muted-foreground/70 uppercase tracking-widest mb-4">Latest Publication</div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 border border-border bg-card hover:bg-muted/30 transition-colors">
                <div className="p-12 lg:p-16 flex flex-col justify-between h-full order-2 lg:order-1">
                  <div>
                    ...
                    <p className="text-muted-foreground font-sans leading-relaxed mb-8 max-w-md">{latest.excerpt}</p>
                  </div>
                ...
                <div className="relative h-64 lg:h-auto border-b lg:border-b-0 lg:border-l border-border bg-background overflow-hidden order-1 lg:order-2 opacity-80 group-hover:opacity-100 transition-opacity">
                  <img alt={latest.title} src={latest.heroImage} className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="border-t border-border">
          ...
          {rest.map((post) => (
            ...
            >
              <div className="md:hidden text-[9px] font-mono text-muted-foreground uppercase tracking-widest mb-1">{post.date}</div>
              ...
              <div className="col-span-6 pr-8">
                <h3 className="text-lg font-medium text-muted-foreground group-hover:text-foreground transition-colors mb-2">{post.title}</h3>
                <p className="text-sm text-muted-foreground leading-relaxed font-sans line-clamp-2">{post.excerpt}</p>
              </div>
