        ...
        {paginatedMCPs.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
            {paginatedMCPs.map((mcp) => (
              <Link
                key={mcp.id}
                href={`/registry/mcp-tools/${mcp.id}`}
          ...
          <div className="mt-8 mb-6">
            ...
            <div className="flex justify-center mt-4">
              <nav className="flex items-center space-x-2">
                ...
                {currentPage > 1 && (
                  <Link 
                    href={`/registry/mcp-tools?query=${encodeURIComponent(query)}&page=${currentPage - 1}&perPage=${perPage}&sortBy=${sortBy}`}
                ...
                {Array.from({ length: endPage - startPage + 1 }, (_, i) => startPage + i).map(num => (
                  <Link 
                    key={num}
                    href={`/registry/mcp-tools?query=${encodeURIComponent(query)}&page=${num}&perPage=${perPage}&sortBy=${sortBy}`}
